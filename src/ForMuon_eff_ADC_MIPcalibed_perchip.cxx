...
Int_t main(int argc,char *argv[])
{
    ...
    tw.forMuon_eff_with_offset_perchip(argv[1],argv[2],argv[3],argv[4],argv[5], trigger_layer0, trigger_layer1, x_offset, y_offset);
...
int raw2Root::forMuon_eff_with_offset_perchip(string str_dat, string str_ped, string str_dac, string str_MIP, string output_file,int trigger_layer0, int trigger_layer1, double* x_offset, double* y_offset){
    ...
    std::cout << "Start Looping over events" << std::endl;
    TEfficiency *efficiency3 = new TEfficiency("efficiency3","efficiency3",Layer_No*chip_No,0,Layer_No*chip_No);
    TEfficiency *efficiency4 = new TEfficiency("efficiency4","efficiency4",Layer_No*chip_No,0,Layer_No*chip_No);
    _triggerIDs.clear();
                ...
                // abs(trigger_layer1_y - trigger1_xy.second) < 20){
                    ...
                    for (int i_layer = 0; i_layer < Layer_No; ++i_layer){
                        ...
                        if ( abs(x_expected - Pos_X(channel,chip)) <  17 && abs(y_expected - Pos_Y(channel,chip)) < 17) {
                            ...
                            bool is_MIP = false;
                            for (int i_channel = 0; i_channel < channel_No; ++i_channel){
                                // if (i_channel == channel) continue;
                                if (MIP_exist.find(std::make_tuple(i_layer,chip,i_channel)) != MIP_exist.end()){
                                    if (MIP_exist[std::make_tuple(i_layer,chip,i_channel)]){
                                        is_MIP = true;
                                        break;
                                    }
                                }
                            ...
                            //     is_MIP = false;
                            // }
                            efficiency3-> Fill(is_MIP, i_layer*chip_No + chip);
                            bool is_Hit = false;
                            for (int i_channel = 0; i_channel < channel_No; ++i_channel){
                                if (MIP_exist.find(std::make_tuple(i_layer,chip,i_channel)) != MIP_exist.end()){
                                    if (MIP_exist[std::make_tuple(i_layer,chip,i_channel)]){
                            ...
                            //     is_Hit = false;
                            // }
                            efficiency4-> Fill(is_Hit, i_layer*chip_No + chip);
    ...
    efficiency3->SetTitle("MIP Efficiency; (Layer*9)+chip; Efficiency");
    efficiency3->Draw();
    c6->Update();
    auto graph = efficiency3->GetPaintedGraph();
    graph->GetXaxis()->SetRangeUser(0, Layer_No*chip_No);
    graph->Draw("AP");
    for (int i_layer = 0; i_layer < Layer_No; ++i_layer){
        TLine *line = new TLine(i_layer*chip_No, 0, i_layer*chip_No, 1);
        line->SetLineColor(kRed);
        line->SetLineStyle(2);
        line->Draw("same");
        TLatex *latex_layer = new TLatex();
        ...
        if (i_layer == trigger_layer0 || i_layer == trigger_layer1){
            latex_layer->SetTextColor(kBlue);
            latex_layer->DrawLatex(i_layer*chip_No+ chip_No*0.3, 1.09, Form("Trigger Layer"));
        }else{
            latex_layer->SetTextColor(kBlack);
        }
        latex_layer->DrawLatex(i_layer*chip_No+ chip_No*0.3, 1.05, Form("Layer %d", i_layer));
    }
    c6->Update();
    c6->Modified();
    c6->SaveAs("MuonCandidate2_offset_efficiency3_layerchip.png");
    TCanvas *c7 = new TCanvas("c7","c7",1200,600);
    efficiency4->SetTitle("Hit Tag Efficiency; (Layer*9)+chip; Efficiency");
    efficiency4->Draw();
    c7->Update();
    auto graph2 = efficiency4->GetPaintedGraph();
    graph2->GetXaxis()->SetRangeUser(0, Layer_No*chip_No);
    graph2->Draw("AP");
    for (int i_layer = 0; i_layer < Layer_No; ++i_layer){
        TLine *line = new TLine(i_layer*chip_No, 0, i_layer*chip_No, 1);
        line->SetLineColor(kRed);
    ...
    c7->Update();
    c7->Modified();
    c7->SaveAs("MuonCandidate2_offset_efficiency4_layerchip.png");
