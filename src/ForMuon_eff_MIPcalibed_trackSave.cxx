...
{
    if (argc < 8) {
        cout << "Usage: " << argv[0] << " <dat_file> <ped_file> <dac_file> <MIP_file> <output_file> <trigger_layer0> <trigger_layer1> [save_option]" << endl;
        return 1;
    }
    ...
    tw.forMuon_eff_with_offset_trackSave(argv[1],argv[2],argv[3],argv[4],argv[5], trigger_layer0, trigger_layer1, x_offset, y_offset);
...
double z_layer_testbeam(int layer) {
    return layer * 29.63;
}
...
int inverse_z_layer_testbeam(double z) {
    if (z < 0 || z >= 1190.7) {
        std::cout << "Error: z out of range: " << z << std::endl;
        return -1; // Error code for out of range
    }
    int layer = static_cast<int>(z / 29.63);
    if (layer < 0 || layer >= 40) {
        std::cout << "Error: layer out of range: " << layer << std::endl;
        return -1; // Error code for out of range
    }
...
TH1D* h_costheta2(int trigger0, int trigger1){
    ...
    for (int i = 0; i < 1e6; ++i) {
        ...
        double x1 = x0 + tantheta * cos(phi) * abs(z_layer_testbeam(trigger0) - z_layer_testbeam(trigger1));
        double y1 = y0 + tantheta * sin(phi) * abs(z_layer_testbeam(trigger0) - z_layer_testbeam(trigger1));
    ...
    for (int i = 1; i <= h2_display->GetNbinsX(); ++i) {
        for (int j = 1; j <= h2_display->GetNbinsY(); ++j) {
            if (h2_display->GetBinContent(i,j) > 0) {
                nHits++;
                double near_z_val = h2_display->GetXaxis()->GetBinCenter(i);
                double pitch = 29.63;
                int layer_idx = TMath::Nint(near_z_val / pitch);
                layer_idx = std::max(0, std::min(layer_idx, Layer_No - 1));
                double z_val = layer_idx * pitch;
...
int raw2Root::forMuon_eff_with_offset_trackSave(string str_dat, string str_ped, string str_dac, string str_MIP, string output_file,int trigger_layer0, int trigger_layer1, double* x_offset, double* y_offset){
    ...
    double chi2perndf;
    tree_in->SetBranchAddress("chi2perndf",&chi2perndf);
    ...
    TH1D *h_nHits = new TH1D("h_nHits","nHits;nHits",100,0,100);
    ...
    TH1D *h_nHits_full = new TH1D("h_nHits_full","nHits ;nHits",100,0,100);
    TH1D *h_nHits_trigger0 = new TH1D("h_nHits_trigger0","nHits with trigger0 MIP; nHits",100,0,100);
    TH1D *h_nHits_trigger1 = new TH1D("h_nHits_trigger1","nHits with trigger1 MIP; nHits",100,0,100);
    TH2D *h2_nHits_chi2_nontriggered = new TH2D("h2_nHits_chi2_nontriggered","nHits vs chi2/ndf for non-triggered layers;nHits;chi2/ndf",100,0,100,100,0,50);
    THStack *hs_nHits = new THStack("hs_nHits","nHits Distribution; nHits; Counts");
    ...
    TTree *tree_MuonCandidate = new TTree("MuonCandidate","MuonCandidate");
    int trigger_ID;
    int event_Time;
    double track_slope_x;
    double track_intercept_x;
    double track_slope_y;
    double track_intercept_y;
    double track_chi2perndf_x;
    double track_chi2perndf_y;
    int nHits_MuonCandidate;
    tree_MuonCandidate->Branch("trigger_layer0",&trigger_layer0,"trigger_layer0/I");
    tree_MuonCandidate->Branch("trigger_layer1",&trigger_layer1,"trigger_layer1/I");
    tree_MuonCandidate->Branch("trigger_ID",&trigger_ID,"trigger_ID/I");
    tree_MuonCandidate->Branch("event_Time",&event_Time,"event_Time/I");
    tree_MuonCandidate->Branch("track_slope_x",&track_slope_x,"track_slope_x/D");
    tree_MuonCandidate->Branch("track_intercept_x",&track_intercept_x,"track_intercept_x/D");
    tree_MuonCandidate->Branch("track_slope_y",&track_slope_y,"track_slope_y/D");
    tree_MuonCandidate->Branch("track_intercept_y",&track_intercept_y,"track_intercept_y/D");
    tree_MuonCandidate->Branch("track_chi2perndf_x",&track_chi2perndf_x,"track_chi2perndf_x/D");
    tree_MuonCandidate->Branch("track_chi2perndf_y",&track_chi2perndf_y,"track_chi2perndf_y/D");
    tree_MuonCandidate->Branch("nHits_MuonCandidate",&nHits_MuonCandidate,"nHits_MuonCandidate/I");
    ...
    // for (int i = 0; i < 1000; ++i){
        ...
        if (trigger0_MIP_exist == 0 || trigger1_MIP_exist == 0){
            // h2_nHits_chi2_nontriggered->Fill(nhits,0); // placeholder for chi2/ndf
        }
        else if (trigger0_MIP_exist > 0 && trigger1_MIP_exist == 0){
            h_nHits_trigger0->Fill(nhits);
            // h_trigger0_xy->Fill(trigger0_xy.first, trigger0_xy.second);
        }
        else if (trigger1_MIP_exist > 0 && trigger0_MIP_exist == 0){
            h_nHits_trigger1->Fill(nhits);
            // h_trigger1_xy->Fill(trigger1_xy.first, trigger1_xy.second);
        }
        nhits = 0;
        if ((trigger0_MIP_exist > 0 && trigger1_MIP_exist > 0)){
            ...
            std::unique_ptr<TH2D> h2_display_zx(new TH2D("display_zx","display_zx",415,-29.63,1214.83,18,-9*40.3,9*40.3));
            std::unique_ptr<TH2D> h2_display_zy(new TH2D("display_zy","display_zy",415,-29.63,1214.83,18,-9*40.3,9*40.3));
            ...
            for (int i_hit = 0; i_hit < cellID->size(); ++i_hit){
                ...
                if(hitE > 0.5 * MIP_E) h2_display_zx->Fill(z_layer_testbeam(layer),x_corrected,hitE);
            }
            double trigger0_z = z_layer_testbeam(trigger_layer0);
            double trigger1_z = z_layer_testbeam(trigger_layer1);
            ...
            for (int i_hit = 0; i_hit < cellID->size(); ++i_hit){
                ...
                if(hitE > 0.5 * MIP_E) h2_display_zy->Fill(z_layer_testbeam(layer),y_corrected,hitE);
            }
            ...
            track_slope_x = std::get<0>(fit_result);
            track_intercept_x = std::get<1>(fit_result);
            track_slope_y = std::get<0>(fit_result2);;
            track_intercept_y = std::get<1>(fit_result2);
            track_chi2perndf_x = std::get<2>(fit_result);
            track_chi2perndf_y = std::get<2>(fit_result2);
            nHits_MuonCandidate = nhits;
            tree_MuonCandidate->Fill();

            // c_2D->SaveAs("MuonCandidate2.pdf");
            // c_2D->SaveAs(Form("MuonCandidate_%d.png",i));
            //denominator events
            // if ( trigger0_MIP_exist == 1 && trigger1_MIP_exist == 1 && 
            if ( std::get<2>(fit_result) < 2 && std::get<2>(fit_result2) < 2
                ...
                // abs(trigger_layer1_y - trigger1_xy.second) < 20){
                    // passing cell 
                    for (int i_layer = 0; i_layer < Layer_No; ++i_layer){
                        double x = std::get<0>(fit_result) * z_layer_testbeam(i_layer) + std::get<1>(fit_result);
                        double y = std::get<0>(fit_result2) * z_layer_testbeam(i_layer) + std::get<1>(fit_result2);
    ...
    TCanvas *c_nHits = new TCanvas("c_nHits","c_nHits",800,600);
    h_nHits_full->SetTitle("nHits; nHits; Counts");
    h_nHits_full->Draw("hist");
    hs_nHits->SetTitle("nHits; nHits; Counts");
    h_nHits->SetFillColor(kBlue-10);
    h_nHits_trigger0->SetFillColor(kRed-10);
    h_nHits_trigger1->SetFillColor(kGreen-10);
    hs_nHits->Add(h_nHits);
    hs_nHits->Add(h_nHits_trigger0);
    hs_nHits->Add(h_nHits_trigger1);
    hs_nHits->Draw("hist same");
    TLegend *leg_nHits = new TLegend(0.6,0.6,0.85,0.85);
    leg_nHits->AddEntry(h_nHits_full,"full data","f");
    leg_nHits->AddEntry(h_nHits,"MuonCandidate","f");
    leg_nHits->AddEntry(h_nHits_trigger0,"trigger layer 0 only","f");
    leg_nHits->AddEntry(h_nHits_trigger1,"trigger layer 1 only","f");
    leg_nHits->SetBorderSize(0);
    leg_nHits->SetFillColor(0);
    leg_nHits->Draw();
    c_nHits->SaveAs("MuonCandidate2_offset_nHits_stacked.png");
    ...
    h_nHits->Write();
    h_nHits_full->Write();
    h_nHits_trigger0->Write();
    h_nHits_trigger1->Write();
    hs_nHits->Write();
    c_nHits->Write();
    // h2_nHits_chi2_triggered->Write();
    // h2_nHits_chi2_nontriggered->Write();
